...
Properties {
	...
	_SparkWidth ("Spark Width" , range(0.001,0.499)) = 0.250
}
...
half4 frag( v2f i ) : COLOR
{
	...
	float2 noiseVec = float2(i.uvmain.x / 5,abs(sin(_Time.y + _StartSeed)) * 256);	
	float4 noiseSamp = tex2D( _Noise,noiseVec);
		
	float dvdr = 1.0 - abs(i.uvmain.x - 0.5) * 2;
	dvdr = clamp(dvdr+_SparkDist,0,1);
	
	float fullWidth = 1 - _SparkWidth * 2;
	// Center the scaled texture
	float scaledTexel = (i.uvmain.y - _SparkWidth) / fullWidth;
			
	float offs = scaledTexel + ((0.5 - noiseSamp.y)/2) * _SparkMag * dvdr;
	offs = clamp(offs,0,1);
			
	
	float2 texSampVec = float2(offs,i.uvmain.x);
